<template>
  <div>
    ...
    <div class="form-card">
                ...
                <q-list separator>
                  <q-item v-for="patient in filteredSearchResults" :key="patient.patient_id" clickable
                    :disable="!patient.eligible" @click="selectPatientFromDropdown(patient)" :class="{
                      'dropdown-patient-item': true,
                      'patient-eligible': patient.eligible,
                      'patient-ineligible': !patient.elegant
                    }">
                    <q-item-section>
                      <q-item-label class="patient-name-dropdown">
                        {{ formatPatientName(patient) }}
      ...
      <!-- STEP 1: INSUFFICIENT FUNDS DIALOG -->
      <q-dialog v-model="showInsufficientFundsDialog" persistent>
        <q-card style="min-width: 500px;">
          ...
          <q-card-actions align="right" class="q-px-md q-pb-md q-pt-md">
            ...
            <!-- Proceed anyway → opens the Details dialog next -->
      ...
      <!-- STEP 2: DETAILS / FINAL CONFIRMATION DIALOG -->
      <q-dialog v-model="showFinalConfirmDialog" persistent>
        <q-card style="min-width: 700px; max-width: 800px;">
          ...
          <q-card-section>
            ...
            <!-- No browser patient — show form values -->
            <div v-else>
              <div class="patient-info-box">
                <div class="text-subtitle2 text-weight-bold q-mb-sm text-blue-8">
          ...
          <q-card-actions align="right" class="q-px-md q-pb-md q-pt-md dialog-actions-sticky">
            ...
            <!-- Proceed → opens Are You Sure last -->
      ...
      <!-- STEP 3: ARE YOU SURE DIALOG - absolute last step before submit -->
        ...
        <q-card style="min-width: 500px; max-width: 600px;">
          ...
          <q-card-section>
            <div class="text-subtitle1">
              You are about to save this patient record. This action cannot be undone.
            </div>
            <div class="text-body2 text-grey-8 q-mt-sm">
              Click <strong>CONFIRM</strong> to finalize and save, or <strong>BACK</strong> to review the details again.
            </div>
          </q-card-section>
          ...
          <q-card-actions align="right" class="q-px-md q-pb-md q-pt-md">
            <q-btn label="BACK" icon="arrow_back" unelevated class="dialog-goback-btn"
              @click="showAreYouSureDialog = false; showFinalConfirmDialog = true" />
            <q-btn label="CONFIRM" icon="check" unelevated class="dialog-cancel-btn"
              @click="confirmAreYouSure" :loading="areYouSureLoading" />
          </q-card-actions>
        </q-card>
      </q-dialog>
...
const formatAddress = (patient) => {
  const parts = [patient.house_address, patient.barangay, patient.city, patient.province].filter(Boolean)
...
const onLastNameBlur = () => {
  setTimeout(() => {
    ...
    if (!usedBrowserPatient.value) showPatientDropdown.value = false
  }, 200)
}
...
// STEP 2 → STEP 3: Details confirmed, open Are You Sure
...
// STEP 1 → STEP 2: Acknowledged insufficient funds, now show details
const proceedWithInsufficientFunds = () => {
  showInsufficientFundsDialog.value = false
  showFinalConfirmDialog.value = true
}

// STEP 3: Final confirmed — submit
...
const checkExistingPatients = async (isPrint = false) => {
  ...
  // Always go through budget check first (which routes to the correct step)
  await checkBudget()
}
...
const submitForm = async (shouldPrint, patientId = null, updatePatientInfo = false) => {
  ...
  if (!mysqlBirthdate) {
    $q.notify({ type: 'negative', message: 'Invalid birthdate format', position: 'top' })
    return
  }
  ...
  if (patientId) {
    ...
    if (updatePatientInfo) formData.append('update_patient_info', '1')
  }
...
const generatePDF = async () => {
  ...
  try {
    ...
    page.drawText(glNum.value + ' / ' + partnerValue.value, { x: 600, y: 489, size: 14, color: rgb(0, 0, 0), font: boldFont })
    page.drawText(fullNameValue.toUpperCase(), { x: 160, y: 375, size: 13, color: rgb(0, 0, 0), font: boldFont })
    if (age !== null) page.drawText(String(age), { x: 545, y: 375, size: 13, color: rgb(0, 0, 0), font: boldFont })
    page.drawText(sexValue.value.toUpperCase(), { x: 630, y: 375, size: 13, color: rgb(0, 0, 0), font: boldFont })
    ...
    const amountX = categoryValue.value === 'MEDICINE' ? 310 : 360
    page.drawText(amountWords, { x: amountX, y: 270, size: 13, color: rgb(0, 0, 0), font: boldFont })
    page.drawText(formatCurrency(issuedAmountValue.value), { x: 340, y: 242, size: 14, color: rgb(0, 0, 0), font: boldFont })
    ...
    page.drawText(issuedByValue.value.toUpperCase(), { x: 340, y: 65, size: 13, color: rgb(0, 0, 0), font: boldFont })
...
.grid-2 { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin-bottom: 14px; }
